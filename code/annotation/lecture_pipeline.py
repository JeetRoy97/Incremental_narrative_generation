...
REWRITE_PROMPT_TEMPLATE = """Instruction: You are an expert educational content designer.
Rewrite a raw lecture transcript into a high-quality educational narrative for
a university level student. The goal is not summarization but faithful
pedagogical rewriting.

Objective: Transform the transcript into a coherent learner-centered narrative
that preserves all content while improving clarity, reasoning, and conceptual
flow.

Learner Alignment: Assume prior knowledge of a university level student. Start
from their mental model and clarify or correct misconceptions when needed.

Narrative Structure: Follow progression: intuition -> explanation ->
formalization -> insight. Introduce tension and resolve it through
explanation.

Causal Flow: Ensure every idea is linked with explicit reasoning (e.g.,
"therefore", "because", "this implies"). Avoid isolated facts.

Information Preservation: Preserve all facts, examples, and steps. Do not
remove or compress content. Maintain full semantic equivalence.

Progressive Explanation: Move from concrete to abstract. Provide intuition
before formal definitions. Ensure smooth conceptual buildup.

Pedagogical Enrichment: Make implicit reasoning explicit. Explain why concepts
matter and what would fail without them. Do not introduce new facts.

Style: Write in continuous prose (no bullets). Tone should be clear, calm, and
tutor-like. Avoid repetition of original phrasing.

Quality Check: Ensure every sentence in the input is represented. Maintain
logical flow and completeness.

Output Format: Return ONLY the rewritten narrative as continuous text. No
headings, metadata, or explanations.
...
def rewrite_narratives(
    ...
    """Rewrite each transcript segment into a learner-centered educational
    narrative, per the exact prompt (Instruction/Objective/Learner
    Alignment/... /Input Transcript). Each row is rewritten independently,
    matching the prompt's single {transcript} input field."""
    df = df.sort_values(["course_id", "lecture_id", "slide_order"]).reset_index(drop=True)
    rewritten = [""] * len(df)

    for i, row in df.iterrows():
        prompt = REWRITE_PROMPT_TEMPLATE.format(transcript=row[transcript_col])
        ...
        for attempt in range(1, cfg.max_retries + 1):
            try:
                ...
                log.warning(
                    "Rewrite failed for course=%s lecture=%s slide_order=%s (attempt %d/%d): %s",
                    row["course_id"], row["lecture_id"], row["slide_order"],
